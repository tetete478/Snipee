...
class GoogleDriveService {
    ...
    private func searchFolder(name: String, completion: @escaping (Result<String?, Error>) -> Void) {
        ...
        let urlString = "\(baseUrl)/files?q=\(encodedQuery)&fields=files(id,name)&supportsAllDrives=true"
    ...
    func findFile(name: String, parentId: String?, completion: @escaping (Result<DriveFile?, Error>) -> Void) {
        ...
        let urlString = "\(baseUrl)/files?q=\(encodedQuery)&spaces=drive&fields=files(id,name,mimeType,modifiedTime)&supportsAllDrives=true"
    ...
    func createFile(name: String, content: Data, parentId: String?, mimeType: String, completion: @escaping (Result<DriveFile, Error>) -> Void) {
        ...
        guard let metadataData = try? JSONSerialization.data(withJSONObject: metadata) else {
            completion(.failure(DriveError.uploadFailed))
            return
        }
    ...
    // MARK: - File Upload (existing file)
    ...
    func getFileMetadata(fileId: String, completion: @escaping (Result<DriveFile, Error>) -> Void) {
        ...
        let urlString = "\(baseUrl)/files/\(fileId)?fields=id,name,mimeType,modifiedTime&supportsAllDrives=true"
