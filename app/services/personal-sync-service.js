...
async function downloadPersonalData() {
  try {
    ...
    if (!file) {
      console.log('[PersonalSync] クラウドにデータなし。ローカルをそのまま使用');
      return;
    }
    ...
    console.log(`[PersonalSync] ダウンロード完了: ${merged.length}件`);
    console.log(`[PersonalSync] 保存フォルダ:`, JSON.stringify(savedFolders));
    console.log(`[PersonalSync] 保存スニペット先頭:`, JSON.stringify(savedSnippets[0]));
...
async function uploadCurrentData() {
  try {
    ...
    if (result) {
      ...
      console.log(`[PersonalSync] アップロード完了: ${snippets.length}件`);
    }
  } catch (error) {
