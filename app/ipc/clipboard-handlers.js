...
module.exports = function registerClipboardHandlers() {
  ...
  ipcMain.handle('get-all-items', () => {
    ...
    return {
      ...
      pinnedItems: appState.clipboard.pinnedItems,
  ...
  ipcMain.handle('clear-all-history', () => {
    ipcMain.emit('log', null, `[DEBUG] pinnedItems: ${JSON.stringify(appState.clipboard.pinnedItems)}`);
    ipcMain.emit('log', null, `[DEBUG] history ids: ${JSON.stringify(appState.clipboard.history.map(i => i.id))}`);
    appState.clipboard.history = appState.clipboard.history.filter(item =>
      appState.clipboard.pinnedItems.includes(item.id)
    );
    store.set('clipboardHistory', appState.clipboard.history);
    return true;
  });
