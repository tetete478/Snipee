...
module.exports = function registerSnippetHandlers() {
  ...
  ipcMain.handle('save-master-snippet', (event, snippet) => {
    if (!memberManager.isAdmin()) return { success: false, error: '権限がありません' };
    const masterSnippets = store.get('masterSnippets', { snippets: [] });
  ...
  // ネイティブコンテキストメニュー（右クリック）
  ipcMain.on('show-context-menu', (event, { type, id, isMaster, folderName }) => {
    const { Menu, BrowserWindow } = require('electron');
    const isAdmin = memberManager.isAdmin();
    const send = (action) => () => event.sender.send('context-menu-action', { action, id, isMaster, folderName });

    let template = [];

    if (type === 'folder') {
      template = [
        { label: '新規スニペット', click: send('add-snippet') },
        { label: '新規フォルダ',   click: send('add-folder') },
        { label: '名前を変更',     click: send('rename-folder') },
        ...(isAdmin ? [
          { type: 'separator' },
          { label: isMaster ? '個別に降格' : 'マスタに昇格', click: send(isMaster ? 'demote-folder' : 'promote-folder') }
        ] : []),
        { type: 'separator' },
        { label: '削除', click: send('delete-folder') }
      ];
    } else {
      template = [
        { label: '新規スニペット', click: send('add-snippet') },
        { label: '名前を変更',     click: send('rename-snippet') },
        ...(isAdmin ? [
          { type: 'separator' },
          { label: isMaster ? '個別に降格' : 'マスタに昇格', click: send(isMaster ? 'demote-snippet' : 'promote-snippet') }
        ] : []),
        { type: 'separator' },
        { label: '削除', click: send('delete-snippet') }
      ];
    }
